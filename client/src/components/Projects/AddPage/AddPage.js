...
class AddPage extends Component {
  ...
  onSubmit = async e => {
    ...
    try {
      ...
      if (!this.state.isEdit || (this.state.isEdit && this.state.pageForm.image !== '')) {
      ...
      if (!this.state.isEdit) {
        ...
        res = await axios.put(`/api/pages/${this.props.page._id}`, page, config);
      }
  ...
  render() {
    return (
      <div className={classes.AddPageContainer}>
        <div>{this.state.isEdit ? 'Edit' : 'Add'} Page {this.state.isEdit && this.props.page._id !== '5e34a6c0ef581f9be4741ef5' && this.props.canDelete ?<span><IconButton onClick={this.props.onDeletePage}><DeleteIcon /></IconButton></span> : ''}</div>
